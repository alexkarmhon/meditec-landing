...
const createProductCard = ({
  ...
  const specialBadge = isSpecial ? `<span>(Спеціальна пропозиція)</span>` : `<span>(${volume})</span>`;
  const priceInfo = isSpecial ? `<span class="product-card__button-price"></span> Замовити` : `<span class="product-card__button-price">Ціна:</span> ${price} грн.`;
  const discountLable = isSpecial ? `<div class="discount">
...
const content = patents.reduce((acc, category) => {
  Object.entries(category).forEach(([key, patentItems]) => {
    ...
    title.textContent = key.includes("UA") ? "База патентів України" : "База патентів СРСР";
